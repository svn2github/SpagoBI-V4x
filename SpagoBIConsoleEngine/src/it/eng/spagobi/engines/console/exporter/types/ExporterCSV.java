...
public class ExporterCSV extends Exporter {
	...
	public void fillHeader(CSVDocument csvDoc) {
		...
		logger.debug("Method fillHeader(): Columns number = " + column);
		...
		for (int j = 0; j < column; j++) {
			...
			logger.debug("Method fillHeader(): Process field [ " + fieldName + " ]");
			...
			if (extractedFields == null || extractedFields.size() == 0) {
				logger.debug("Method fillHeader(): No valid exctracted fields vector!");
			}

			if (extractedFields != null && extractedFields.get(j) != null) {

				logger.debug("Method fillHeader(): Field [ " + fieldName + " ] found in extracted fields");
	...
	public void fillData(CSVDocument csvDoc) {
		...
		logger.debug("Method fillData(): Rows number = " + rowsNumber);
		...
		for (int i = rownum; i < rowsNumber; i++) {
			...
			logger.debug("Method fillData(): Extracted Fields Meta Data size is " + length);

			for (int fieldIndex = 0; fieldIndex < length; fieldIndex++) {
				IFieldMetaData metaField = extractedFieldsMetaData.get(fieldIndex);
				IField f = record.getFieldAt((Integer) metaField.getProperty("index"));
			...
			logger.debug("Method fillData(): Adding a valid row..");
